...
public class Lector
{
    private string[] ingresos = new  string[] {"incorrrecto","paco.txt"}; // no es conveniente pedir datos al usuario, por lo que modifique un poco el codigo para que use valores pre definidos

    
    public string  Busqueda(string dir, int opcion) // esta funcion se encarga de buscar el archivo de texto que agregaste, en base a su nombre, el cual deberas proporcionarle
    {
        ...
        String? input = ingresos[opcion]; // cambio esta parte por un opcion predefinida, que se escojera segun el numero en opcion
    ...
    public  int Vocales( ref string texto, char letra) // en esta funcion realizamos la busqueda de las vocales repetidas
    {
        int cont = 0, punto = 0;
        // para fines de prueba de estas funciones, es mas conveniente revisar y normalizar el texto aqui 
    ...
    public string Palindromear(string texto) // aqui realizaremos la busqueda del palindromo mas largo de todo el texto
    {
        // para fines de prueba de estas funciones, es mas conveniente revisar y normalizar el texto aqui 
        texto = texto.ToLower(); // hacemos que todo el texto este en minusculas 
        texto = Regex.Replace(texto.Normalize(NormalizationForm.FormD), @"[\u0300-\u0301-\u0302]+", ""); // borramos los acentos, y la ^ buscandolos directamente por su unicode para manejar mejor el texto
